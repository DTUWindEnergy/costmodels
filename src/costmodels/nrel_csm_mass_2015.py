...
class LowSpeedShaftMass(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        outputs["lss_mass"] = (
            lss_mass_coeff * (blade_mass * machine_rating * 1e-3) ** lss_mass_exp
            + lss_mass_intercept
        )
...
class GearboxMass(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input(
            ...
            200.0,
            units="N*m/kg",
            desc="In 2024, modern 5-7MW gearboxes are able to reach 200 Nm/kg",
        )
    ...
    def compute(self, inputs, outputs):
        ...
        outputs["gearbox_mass"] = (
            inputs["rotor_torque"] * 1.0e3 / inputs["gearbox_torque_density"]
        )
...
class GeneratorMass(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        outputs["generator_mass"] = (
            generator_mass_coeff * machine_rating + generator_mass_intercept
        )
...
class TransformerMass(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        outputs["transformer_mass"] = (
            transformer_mass_coeff * machine_rating + transformer_mass_intercept
        )
